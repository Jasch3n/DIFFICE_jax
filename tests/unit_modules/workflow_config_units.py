...
def test_runtime_env_gpu_platform_normalizes_to_cuda(monkeypatch):
    monkeypatch.delenv("JAX_PLATFORMS", raising=False)
    monkeypatch.delenv("JAX_PLATFORM_NAME", raising=False)

    env = apply_runtime_env({"jax_platform": "gpu"})

    assert env["JAX_PLATFORMS"] == "cuda"
    assert env["JAX_PLATFORM_NAME"] == "cuda"


def test_runtime_env_precision_maps_to_jax_enable_x64(monkeypatch):
    monkeypatch.delenv("JAX_ENABLE_X64", raising=False)

    env = apply_runtime_env({"precision": "double"})
    assert env["JAX_ENABLE_X64"] == "true"
    assert os.environ["JAX_ENABLE_X64"] == "true"

    env = apply_runtime_env({"precision": "single"})
    assert env["JAX_ENABLE_X64"] == "false"
    assert os.environ["JAX_ENABLE_X64"] == "false"


def test_runtime_env_precision_accepts_aliases(monkeypatch):
    monkeypatch.delenv("JAX_ENABLE_X64", raising=False)

    assert apply_runtime_env({"precision": "float64"})["JAX_ENABLE_X64"] == "true"
    assert apply_runtime_env({"precision": "fp32"})["JAX_ENABLE_X64"] == "false"


def test_runtime_env_precision_rejects_unknown_value():
    with pytest.raises(ValueError, match="runtime.precision"):
        apply_runtime_env({"precision": "half"})


def test_runtime_env_typo_warns_and_maps(monkeypatch):
    monkeypatch.delenv("JAX_PERSISTENT_CACHE_MIN_COMPILE_TIME_SECS", raising=False)

    with pytest.warns(RuntimeWarning, match="jax_persistence_cache_min_compile_time_secs"):
        env = apply_runtime_env({"jax_persistence_cache_min_compile_time_secs": 0})

    assert env["JAX_PERSISTENT_CACHE_MIN_COMPILE_TIME_SECS"] == "0"


def test_runtime_env_correct_key_wins_over_typo(monkeypatch):
    monkeypatch.delenv("JAX_PERSISTENT_CACHE_MIN_COMPILE_TIME_SECS", raising=False)

    with pytest.warns(RuntimeWarning, match="jax_persistence_cache_min_compile_time_secs"):
        env = apply_runtime_env(
            {
                "jax_persistent_cache_min_compile_time_secs": 3,
                "jax_persistence_cache_min_compile_time_secs": 0,
            }
        )

    assert env["JAX_PERSISTENT_CACHE_MIN_COMPILE_TIME_SECS"] == "3"


def test_kfac_batch_signature_accepts_same_shape_dtype():
    batch = {"col": [[jnp.ones((2, 2), dtype=jnp.float32)]], "step": jnp.array(1, dtype=jnp.int32)}
    signature = solver_module._batch_leaf_signature(batch)

    assert solver_module._check_batch_leaf_signature(signature, batch, "same") == signature


def test_kfac_batch_signature_rejects_shape_leaf_and_dtype_changes():
    batch = {"col": [[jnp.ones((2, 2), dtype=jnp.float32)]], "step": jnp.array(1, dtype=jnp.int32)}
    signature = solver_module._batch_leaf_signature(batch)

    with pytest.raises(ValueError, match="KFAC batch structure changed"):
        solver_module._check_batch_leaf_signature(
            signature,
            {"col": [[jnp.ones((3, 2), dtype=jnp.float32)]], "step": jnp.array(1, dtype=jnp.int32)},
            "shape",
        )
    with pytest.raises(ValueError, match="KFAC batch structure changed"):
        solver_module._check_batch_leaf_signature(
            signature,
            {"col": [[jnp.ones((2, 2), dtype=jnp.float32)]]},
            "missing",
        )
    with pytest.raises(ValueError, match="KFAC batch structure changed"):
        solver_module._check_batch_leaf_signature(
            signature,
            {"col": [[jnp.ones((2, 2), dtype=jnp.int32)]], "step": jnp.array(1, dtype=jnp.int32)},
            "dtype",
        )


def test_kfac_compile_signature_excludes_paths_and_job_metadata(monkeypatch, tmp_path):
    ...
    solver = djax.DIFFICESolver(
        ...
        model=djax.ModelConfig(
            ...
            network=djax.NetworkConfig(depth=3, width=9),
        ),
        loss=djax.LossConfig(name="joint_inversion", use_gpinn=True, active_regions=[0, 1]),
    )
    stage = djax.TrainingStage(
        ...
        adaptive_sampling_period=2,
    )
    batch_signature = solver_module._batch_leaf_signature({"col": [[jnp.ones((2, 2), dtype=jnp.float32)]]})

    signature = solver._kfac_compile_signature(
        batch_signature,
        {
            "learning_rate": None,
            "damping": jnp.nan,
    ...
    dumped = json.dumps(signature, sort_keys=True)

    assert str(tmp_path) not in dumped
    assert "SLURM_JOB_ID" not in dumped
    assert signature["jax_cache_env"]["JAX_COMPILATION_CACHE_DIR"] == "<set>"
