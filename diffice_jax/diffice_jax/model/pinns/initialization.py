...
def init_single_net(parent_key, layer_widths, embedding=False, embed_n=5, embed_std=3.0):
    ...
    # and zero-initialize the final linear layer to prevent huge initial PDE residuals
    if embedding:
        # zero-initialize the final linear layer (which is the last one added to params)
        params[-1][0] = jnp.zeros_like(params[-1][0])
        params[-1][1] = jnp.zeros_like(params[-1][1])
...
def init_nets(parent_key, n_hl, n_unit, aniso=False, basal=False, embedding=False, embed_n=10, embed_std=1.0):
    ...
    if basal:
        ...
        params_mu = init_single_net(keys[1], layers2, embedding=embedding, embed_n=embed_n, embed_std=embed_std)
        params_c = init_single_net(keys[2], layers3, embedding=embedding, embed_n=embed_n, embed_std=embed_std)
