...
% Compatibility field for older L-curve-oriented helpers. New configs are
% single-weight; an L-curve should be produced by sweeping config files.
config.lcurve_regularization_weights = config.regularization_weight;
...
config.lcurve_path = fullfile(results_dir, ...
    sprintf('%s_lcurve_rheology_B.mat', shelf_name));
...
config.lcurve_plot_path = fullfile(results_dir, ...
    sprintf('%s_lcurve_rheology_B.png', shelf_name));
...
if isstruct(value) && isfield(value, 'use_lcurve_middle') && ...
        value.use_lcurve_middle
    weight = regularization_weight;
    return;
end
