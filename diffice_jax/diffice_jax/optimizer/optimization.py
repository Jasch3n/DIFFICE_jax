...
@functools.partial(jit, static_argnames=("lossf", "opt", "use_ibpinn"))
def adam_minimizer(lossf, params, data, opt, opt_state, use_ibpinn=False):
    """Basic gradient update step based on the opt optimizer."""
    
    if use_ibpinn:
        # Expected params = (log_eps, net_params) when use_ibpinn is True
        grads, loss_info = grad(lossf, has_aux=True)(params, data)
        # grads is a tuple (grad_log_eps, grad_net_params)
        updates, opt_state = opt.update(grads, opt_state)
        # Apply updates to log_eps and net_params separately
        new_log_eps = optax.apply_updates(params[0], updates[0])
        new_net_params = optax.apply_updates(params[1], updates[1])
        new_params = (new_log_eps, new_net_params)
    else:
        grads, loss_info = grad(lossf, has_aux=True)(params, data)
        updates, opt_state = opt.update(grads, opt_state)
...
def adam_optimizer(key, lossf, params, dataf, epoch, lr=1e-3, aniso=False, schdul=None,
    ...
    # optain the initial state of the params
    
    if use_ibpinn:
        # Initialize epsilon variables
        log_eps = {
            'd': jnp.log(jnp.array(eps_init)),
            'e': jnp.log(jnp.array(eps_init)),
            'b': jnp.log(jnp.array(eps_init)),
        }
        # Pack into a joint structure
        params = (log_eps, params)
    ...
    for step in range(epoch):
        ...
        if run_RAD:
            ...
            adapted=True
            net_params = params[1] if use_ibpinn else params
            data = dataf(key, eval_adaptive=True, eval_f=lambda x, idx, basal: eval_f(net_params, x, idx, basal))
        ...
        params, loss_info, opt_state = adam_minimizer(lossf, params, data, opt_Adam, opt_state, use_ibpinn=use_ibpinn)
        ...
        if (step+1) % 500 == 0:
            ...
            if use_ibpinn:
                log_eps = params[0]
                omega_d = 1.0 / (2 * jnp.exp(log_eps['d'])**2)
                omega_e = 1.0 / (2 * jnp.exp(log_eps['e'])**2)
                omega_b = 1.0 / (2 * jnp.exp(log_eps['b'])**2)
                print(f"ADAM Step:{step+1} | Loss:{loss_info[0]:.4e} | d:{loss_info[1]:.4e} | eq:{loss_info[2]:.4e} | "
                      f"bd:{loss_info[3]:.4e} | w_d:{omega_d:.2e} | w_e:{omega_e:.2e} | w_b:{omega_b:.2e}", file=sys.stderr)
            elif len(net_params) > 1:
                print(f"ADAM Step:{step+1} | Loss:{loss_info[0]:.4e} | d:{loss_info[1]:.4e} | eq:{loss_info[2]:.4e} | "
    ...
    # Return params with the log_eps dict if IBPINN was used, so it can be passed to L-BFGS.
    # The caller will need to decide whether to unpack it.
    if adaptive:
        return params, loss_all# , probs_last 
    else:
        return params, loss_all

def lbfgs_function(lossf, init_params, data, basal=False, print_rate=500, use_ibpinn=False, fixed_log_eps=None):
    """
    ...
    def f(params_1d):
        ...
        unravelled_params = unravel(params_1d)
        
        # If IBPINN is used, repack the network params with the frozen log_eps
        if use_ibpinn and fixed_log_eps is not None:
            params = (fixed_log_eps, unravelled_params)
        else:
            params = unravelled_params
        ...
        # If IBPINN was used, _value_and_grad will return a tuple of gradients (eps_grads, param_grads).
        # We only want to ascend the param_grads which match the input params_1d.
        if use_ibpinn and fixed_log_eps is not None:
            grads_1d = flat_utl.ravel_pytree(grads[1])[0]
        else:
...
def lbfgs_optimizer(lossf, params, data, epoch, basal=False, print_rate=1000, use_ibpinn=False, fixed_log_eps=None):
    """
    ...
    func_lbfgs = lbfgs_function(
        lossf, params, data, basal=basal, print_rate=print_rate, use_ibpinn=use_ibpinn, fixed_log_eps=fixed_log_eps
    )
